...
5. **Document** — write the requirements spec and task breakdown (format below) to `./docs/plans/<YYYY-MM-DD>-<slug>.md` in the project. Chat output dies with the context window; the file is what post-compaction agents, /execute subagents, and human reviewers read. Create the directory if needed, as it is ignored globally in `~/.config/git/ignore`. Reply with the file path and a short summary, not the full document.

---
name: execute
description: Delegate a task's implementation wholesale to OpenAI Codex (`codex-run work`) — the whole requirements document in one long autonomous run — keeping policy, orchestration, and verification in the main loop. Use at the Implement step of the operating loop, for any multi-part build.
...
3. **Monitor** — tail `--log`; don't interrupt a healthy run. The stderr banner prints `session: <id>` and a ready-to-run `--resume` line; resuming keeps that session's context, so prefer it to a cold re-delegate — but never resume a session another run is still writing. Exit `3` = turn finished, report STATUS not COMPLETE → resume to finish, or re-delegate citing its checkpoint commits. Other non-zero = turn failed or never launched → read stderr and the log tail; partial edits may remain (`git status`/`git log`). `--resume` also recovers a transient failure (network drop, timeout, kill); cold re-delegate only when the approach itself was wrong. Recurring failures → switch approach per the error-handling rules.
4. **Verify** — never infer success from an exit code or a fluent summary. Check Codex's requirement-by-requirement audit against the plan file, and run the tests/build/lint yourself. Then `feedback`, plus `qa-review` when there's runnable behavior — analysis only; tell each to skip its close-the-loop phase, since this step owns the fix loop. Fix critical and major findings: trivial ones directly, substantial ones as a new `codex-run work` brief citing them. Report the commands and results; don't finish with critical or major findings open.
